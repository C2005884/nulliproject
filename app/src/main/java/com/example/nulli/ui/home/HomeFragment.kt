                ...
                for(s in snapshot.children) {
                    when(s.key) {
                        ...
                        BoardListActivity.EXTERNAL_DISABLED_BOARD -> {
                            boardPair[1] = Pair("외부장애 게시판", s.child("content").getValue(String::class.java).toString())
                        }
                        BoardListActivity.INTERNAL_DISABLED_BOARD -> {
                            boardPair[2] = Pair("내부장애 게시판", s.child("content").getValue(String::class.java).toString())
                        }
                        BoardListActivity.DEVELOP_DISABLED_BOARD -> {
                            boardPair[3] = Pair("발달장애 게시판", s.child("content").getValue(String::class.java).toString())
                        }
                        BoardListActivity.MENTALITY_DISABLED_BOARD -> {
                            boardPair[4] = Pair("정신장애 게시판", s.child("content").getValue(String::class.java).toString())
                        }
                    }
                }
                ...
                clickEvent = {type, position ->
                    if (type == "board") {
                        ...
                        requireActivity().startActivity(
                            Intent(
                                requireActivity(), BoardListActivity::class.java
                            ).apply {
